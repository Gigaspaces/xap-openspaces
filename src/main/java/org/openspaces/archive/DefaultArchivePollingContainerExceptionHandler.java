 ...
 * @author Itai Frenkel
 *
 * Logs exceptions from the {@link ArchivePollingContainer}
 * Filters last exception so it won't be repeatedly logged more than once per minute.
 */
public class DefaultArchivePollingContainerExceptionHandler implements EventExceptionHandler<Object> {
    ...
    /**
     * This is the default handler if {@link #setExceptionHandler(EventExceptionHandler)} was not called.
     * Does nothing in case of a successful archive operation
     */
    @Override
    ...
    /**
     *  This is the default handler if {@link #setExceptionHandler(EventExceptionHandler)} was not called.
     *  Logs the exception if it has not occured in the last minute.
     */
    @Override
