...
class DefaultContainersSlaEnforcementEndpoint implements ContainersSlaEnforcementEndpoint {
    ...
    private void terminateOrphanContainersOfAgent(final GridServiceAgent agent, final FutureGridServiceContainer suspectedFuture) {
    	final int suspectedOrphanContainerAgentId;
    	try {
        	suspectedOrphanContainerAgentId = suspectedFuture.getAgentId();
        }
        catch (TimeoutException e) {
        	return; // container not even started
        }
    	catch (ExecutionException e) {
    		return; // container not even started
    	}
            ...
            public void run() {
                ...
                try {
                    ...
                    logger.warn("Terminated orphan container that did not register with lookup service on machine "
                            + ContainersSlaUtils.machineToString(agent.getMachine()) + " agentId=" + suspectedOrphanContainerAgentId);
                    ...
                    logger.warn("Error terminating orphan container that did not register with lookup service on machine "
                            + ContainersSlaUtils.machineToString(agent.getMachine()) + " agentId=" + suspectedOrphanContainerAgentId, e);
                }
