...
public class StatelessHibernateSpaceSynchronizationEndpoint extends AbstractHibernateSpaceSynchronizationEndpoint {
    ...
    /**
     * Perform the given transation changes using Hibernate {@link org.hibernate.StatelessSession}. First, tries to perform
    ...
    /**
     * Perform the given batch changes using Hibernate {@link org.hibernate.StatelessSession}. First, tries to perform
     * "optimistic" operations without checking in advance for existence of certain entity. If this fails, will
     * try and perform the same operations again, simply with checking if the entry exists or not.
     */
    @Override
    ...
    private void executePartialUpdate(StatelessSession session, DataSyncOperation dataSyncOperation) {
        ...
        query.setParameter("id_" + typeName ,dataSyncOperation.getDataAsDocument().getProperty(typeDescriptor.getIdPropertyName()));
        query.executeUpdate();
    }
