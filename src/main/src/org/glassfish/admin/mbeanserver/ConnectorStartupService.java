 ...
 * GIGAPSACES CHANGES:
 *
 * 1. Be able to not create RMI Registry, but just get it.
 */
    ...
    private static Registry startRegistry(final int port) {
        try {
            try {
                ...
                // GIGAPSACES : Try and get the registry if can't create it
            ...
            public JMXConnectorServer startRMIConnector( final String name) throws IOException
            {
               ...
               // env.put("jmx.remote.protocol.provider.pkgs", "com.sun.jmx.remote.protocol");
                ...
                try {
                    ...
                    // GIGAPSACES: If we alreayd have an mbean server, just boot
                    if (e.getCause() instanceof NameAlreadyBoundException) {
                        // it is already bound, simply boot ourself
                        mAMXBooter.bootAMX();
                    } else {
                        throw e;
                    }
