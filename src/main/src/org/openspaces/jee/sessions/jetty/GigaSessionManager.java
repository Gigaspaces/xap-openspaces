...
public class GigaSessionManager extends org.mortbay.jetty.servlet.AbstractSessionManager {
            ...
            try {
                ...
                if (data == null) {
                    ...
                    if ((oldSession == null) || (data.getAccessed() > oldSession._data.getAccessed())) {
                        ...
                        session.didActivate();
                        if (Log.isDebugEnabled())
                            Log.debug("Refreshed in-memory Session with " + data.toStringExtended());
                    } else {
                        if (Log.isDebugEnabled())
                            Log.debug("Not updating session " + idInCluster + ", in-memory session is as fresh or fresher");
                        session = oldSession;
                    }
                }
    ...
    protected void scavenge() {
        ...
        try {
            ...
            if (Log.isDebugEnabled())
                Log.debug("Scavenger running at " + now + " for context = " + _context.getContextPath());
            ...
            synchronized (this) {
                ...
                if ((_scavengeCount % 2) == 0) {
                    if (Log.isDebugEnabled())
                        Log.debug("Scavenging old sessions, expiring before: " + (now - (2 * _scavengePeriodMs)));
