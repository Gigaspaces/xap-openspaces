...
/**
 * <p>A Space mode based Spring context loader allows to load Spring application context if the Space
 ...
 * <code>PRIMARY</code> mode which basically applies when directly working with cluster members and not a clsutered
 * space proxy (since in such cases it will always be <code>PRIMARY</code>).
 *
 * <p>The context loader accepts a Spring {@link org.springframework.core.io.Resource} as the location. A flag
 * called {@link #setActiveWhenPrimary(boolean)} which defaults to <code>true</code> allows to control if the
 * context will be loaded only when the cluster member moves to <code>PRIMARY</code> mode.
 *
 * @author kimchy
 */
public class SpaceModeContextLoader implements ApplicationContextAware, InitializingBean, DisposableBean,
    ...
    /**
     * Controls if the Spring context will be loaded when the space cluster member moves to <code>PRIMARY</code>
     * mode. Defaults to <code>true</code>.
     */
    ...
    /**
     * Used to pass {@link org.openspaces.core.cluster.ClusterInfo} to the newly created application
     * context.
     */
    ...
    /**
     * Injected by Spring and used as the parent application context for the newly created application
     * context.
     */
    ...
    /**
     * If {@link #setActiveWhenPrimary(boolean)} is set to <code>true</code> (the default) will listens for
     * {@link org.openspaces.core.space.mode.AfterSpaceModeChangeEvent} and load an application context if
     * received.
     */
    ...
    /**
     * Loads the application context and adding specific bean factory and bean post processors. Won't
     * load an application context if one is already defined.
     */
