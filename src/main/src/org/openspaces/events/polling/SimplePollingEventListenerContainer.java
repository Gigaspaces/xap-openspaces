...
/**
 * <p>Event listener container variant that uses plain Space take API, specifically
 * a loop of {@link org.openspaces.core.GigaSpace#take(Object,long)}  calls that also allow for
 * transactional reception of messages.
 *
 * <p>Actual event listener execution happens in asynchronous work units which are
 ...
 * 5 concurrent consumers in case of increasing event load, as well as dynamic
 ...
 * and down. Note that using more than one consumer might break fifo behviour if fifo
 * is configured by the space or the specific class type.
 *
 * @author kimchy
 */
public class SimplePollingEventListenerContainer extends AbstractPollingEventListenerContainer {
    ...
    /**
     * <p>Set the Spring {@link org.springframework.core.task.TaskExecutor} to use for running the listener threads.
     * Default is {@link org.springframework.core.task.SimpleAsyncTaskExecutor}, starting up a number of new threads,
     * according to the specified number of concurrent consumers.
     *
     * <p>Specify an alternative TaskExecutor for integration with an existing thread pool. Note that this really
     * only adds value if the threads are managed in a specific fashion, for example within a J2EE environment.
     * A plain thread pool does not add much value, as this listener container will occupy a number of threads
     ...
     * to scale the consumption of events. However, note that any ordering guarantees are
     * lost once multiple consumers are registered. In general, stick with 1 consumer for
     * low-volume events.
     *
     ...
     *
     * <p>This returns the currently configured "concurrentConsumers" value; the number of currently scheduled/active
     ...
     *
     * <p>Raising the number of concurrent consumers is recommendable in order
     * to scale the consumption of events. However, note that any ordering guarantees are lost once
     * multiple consumers are registered. In general, stick with 1 consumer for low-volume events.
     *
     * <p><b>This setting can be modified at runtime, for example through JMX.</b>
     *
     * @see #setConcurrentConsumers
     ...
     *
     * <p>This returns the currently configured "maxConcurrentConsumers" value; the number of currently
     ...
     * More concretely, this limits the number of event reception attempts per
     * task, which includes receive iterations that did not actually pick up a
     * event until they hit their timeout (see "receiveTimeout" property).
     *
     ...
     * received any event within its execution. If this limit is reached,
     * the task will shut down and leave receiving to other executing tasks
     * (in case of dynamic scheduling; see the "maxConcurrentConsumers" setting).
     * Default is 1.
     *
     * <p>Within each task execution, a number of event reception attempts
     * (according to the "maxEventsPerTask" setting) will each wait for an incoming
     * event (according to the "receiveTimeout" setting). If all of those receive
     * attempts in a given task return without an event, the task is considered
     * idle with respect to received events. Such a task may still be rescheduled;
     ...
     *
     * <p>Called once an event has been received, to scale up while
     * processing the event in the invoker that originally received it.
     *
     * @see #setTaskExecutor
     ...
     *
     * <p>The default implementation delegates to <code>refreshConnectionUntilSuccessful</code> which
     ...
     * successful.
     *
     * <p>The default implementation pings the space until a sucessful ping has been established.
     *
    ...
    protected void doShutdown() throws GigaSpaceException {
        ...
        synchronized (this.activeInvokerMonitor) {
            while (this.activeInvokerCount > 0) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Still waiting for shutdown of [" + this.activeInvokerCount + "] event listener invokers");
                }
                try {
