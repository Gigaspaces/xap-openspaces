...
/**
 * <p>A space remoting proxy that forware the service execution to a remote service with the space as the
 * transport layer. Services are remotly exported in the "server side" using the
 * {@link org.openspaces.remoting.SpaceRemotingServiceExporter}. This proxy builds a representation of the
 * remote invocation using {@link org.openspaces.remoting.SpaceRemoteInvocation} and waits for a remoting
 * response represented by {@link org.openspaces.remoting.SpaceRemoteResult}.
 *
 * <p>The proxy requires a {@link #setGigaSpace(org.openspaces.core.GigaSpace)} interface to be set in
 * order to write the remote invocation and wait for a response using the space API. It also requires
 * a {@link #setServiceInterface(Class)} which represents the interface that will be proxied.
 *
 * <p>Allows for one way invocations (i.e. not waiting for a response). The one way invocation can be
 * set globablly for all of the service methods by setting {@link #setGlobalOneWay(boolean)} or can be
 * enabled only for methods that return <code>void</code> by setting {@link #setVoidOneWay(boolean)}.
 * Note, if using one way invocation and an exception is raised by the remote service, it won't be
 * raised by this proxy.
 *
 ...
 * one of the service interface methods returns {@link org.openspaces.remoting.RemoteFuture}, it can be
 * used for async execution. Note, this means that in terms of interfaces there will have to be two
 * different service interfaces (under the same package and with the same name). One for the server side
 * service that returns the actual value, and one on the client side that for the same method simply
 * returns the future.
 *
 * <p>In case of remote invocation over a partitioned space the default partitioned routing index will be
 * random (the hashCode of the newly created {@link org.openspaces.remoting.SpaceRemoteInvocation} class).
...
public class SpaceRemotingProxyFactoryBean extends RemoteAccessor implements FactoryBean, InitializingBean, MethodInterceptor {
     ...
     * for both {@link org.openspaces.remoting.SpaceRemoteInvocation} and {@link org.openspaces.remoting.SpaceRemoteResult}.
     */
    ...
    /**
     * Sets the timeout that will be used to wait for the remote inovocation response. The timeout value
     * is in <b>milliseconds</b> and defaults to <code>5000</code> (5 seconds).
     */
    ...
    /**
     * In case of remote invocation over a partitioned space the default partitioned routing index will be
     * random (the hashCode of the newly created {@link org.openspaces.remoting.SpaceRemoteInvocation} class).
     * This {@link org.openspaces.remoting.RemoteRoutingHandler} allows for custom routing computation (for
     * example, based on one of the service method parameters).
     */
    ...
    /**
     * <p>If set to <code>true</code> (defaults to <code>false</code>) all of the service methods will be
     * invoked and the proxy will not wait for a return value. Note, any exception raised by the remote
    ...
    /**
     * <p>If set to <code>true</code> (defaults to <code>false</code>) service methods that return void
     * will be invoked and the proxy will not wait for a return value. Note, any exception raised by the remote
     * service will be logged on the server side and not propagated to the client.
     */
