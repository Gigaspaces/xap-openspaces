 ...
 *
 * @version $Id: Groovysh.groovy 14449 2008-12-16 15:27:40Z glaforge $
    ...
    extends Shell
{
    ...
    private static Closure createDefaultRegistrar() {
        return { shell ->
            ...
            r.register(Groovysh.getResource('commands/main/commands.xml'))
        }
    }
    ...
    void removeContext() {
        contextItems.remove(contextItems.size() - 1)
        registerCommands()
    }

    private void registerCommands() {
        ContextItem contextItem = contextItems.last()
        def commands = []
        registry.each { item -> commands << item }
        commands.each { item -> registry.remove(item)  }
