...
set LCP=.
for %%i in ("%JSHOMEDIR%\lib\velocity\*.jar") do call %JSHOMEDIR%\bin\lcp %%i
set VELOCITY_JARS=%LCP%


set COMMAND_LINE=%JAVACMD% %JAVA_OPTIONS% %bootclasspath% -Dlb.vmDir="%JSHOMEDIR%/tools/apache" %LOOKUP_LOCATORS_PROP% %LOOKUP_GROUPS_PROP% %RMI_OPTIONS% "-Dcom.gs.home=%JSHOMEDIR%" -Djava.security.policy="%POLICY%" -classpath %PRE_CLASSPATH%;%COMMON_JARS%;%SPRING_JARS%;%EXT_JARS%;%JDBC_JARS%;"%JSHOMEDIR%";"%JSHOMEDIR%\lib\JSpaces.jar";%OPENSPACES_JARS%;%VELOCITY_JARS%;%POST_CLASSPATH% org.openspaces.pu.container.jee.lb.apache.ApacheLoadBalancerAgent %*
