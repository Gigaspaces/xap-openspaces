...
COMMAND_LINE="${JAVACMD} ${JAVA_OPTIONS} -Xmx1024m $bootclasspath ${RMI_OPTIONS} ${LOOKUP_LOCATORS_PROP} ${LOOKUP_GROUPS_PROP} -Djava.security.policy=${POLICY} -Dcom.gs.home=${JSHOMEDIR} -classpath "${COMMON_JARS}${CPS}${SPRING_JARS}${CPS}${EXT_JARS}$CPS${JDBC_JARS}$CPS${JSHOMEDIR}${CPS}${JSHOMEDIR}/lib/JSpaces.jar$CPS${OPENSPACES_JARS}" org.openspaces.pu.container.standalone.StandaloneProcessingUnitContainer $*"
